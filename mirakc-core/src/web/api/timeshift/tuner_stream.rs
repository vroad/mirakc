...
/// session the recorder is using, so it never allocates or grabs a tuner.
///
/// When the recorder is not currently recording, no tuner output is
/// available to observe, so the request fails.
#[utoipa::path(
    get,
    ...
    W: Spawn,
{
    ...
    // If the recorder is not recording, there is no session to observe; we
    // must not fall back to allocating a new tuner in that case.
    let stream_id = recorder
        .tuner_subscription_id
        .ok_or(Error::TunerUnavailable)?;

    // Tap the tuner session that currently feeds the recorder.
    //
    // recorder is only a snapshot from the QueryTimeshiftRecorder call above;
    // recording can stop before the StartStreaming call below reaches the
    // tuner manager.  This race condition should be rare, but it can happen.
    //
    // To guard against it, we pass the recorder's subscription ID, which
    // makes the tuner manager take the "reuse specified tuner" path.  That
    // path reuses the exact session if it is still alive, and fails if the
    // session has already ended, rather than grabbing another tuner.
